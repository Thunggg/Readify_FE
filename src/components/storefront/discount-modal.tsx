...
export function DiscountModal({
  ...
  const getUnavailableReason = (promotion: Promotion): string | null => {
    ...
    // Check usage limit
    if (promotion.usageLimit && promotion.usedCount >= promotion.usageLimit) {
      return "Mã đã hết số lần sử dụng";
    }
  ...
  const handleApplyManualCode = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      // Try to find the promotion with this code
      ...
      if (matchingPromotion) {
        // If found in list, use exact calculation
        handleSelectCode(matchingPromotion);
      } else {
        // If not found in active list, send to backend for validation
        // Backend will validate the code and calculate discount
        toast.info("Validating promotion code...");
        onSelectCode(code, 0);
        onOpenChange(false);
      }
    } catch (error) {
